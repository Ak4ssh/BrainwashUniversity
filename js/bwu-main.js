...
jQuery(function($) {
    ...
    function f(b, g, e, h) {
        ...
        g && clearInterval(a),
        f = b.index() + 1,
        "prev" == e ? b.prev().length > 0 ? (d = b.prev(),
        c = f - 1) : c = (d = b.parent("ul").children("li").last()).index() + 1 : "next" == e ? b.next().length > 0 ? (d = b.next(),
        c = f + 1) : (d = $(".slider_container li").first(),
        c = 1) : "button" == e && (d = $(".slider_container li:nth-of-type(" + (c = h.index() + 1) + ")")),
        b.fadeOut(2e3).promise().done(function() {
            b.removeClass("current_slide"),
            d.addClass("current_slide"),
            d.fadeIn(1500),
            $(".current_slide_counter").removeClass("current_slide_counter"),
            $(".buttons span:nth-of-type(" + f + ")").removeClass("current_slide_counter"),
            $(".buttons span:nth-of-type(" + c + ")").addClass("current_slide_counter")
        })
    }
    function b() {
        f($(".current_slide"), !1, "next")
    }
    $(".slider_container picture").each(function(a) {
        c[a] = new Image,
        c[a].src = $(this).attr("src"),
        d < 1 ? e += $(".buttons").append('<span class="slide_counter_' + d + ' current_slide_counter"></span>') : e += $(".buttons").append('<span class="slide_counter_' + d + '"></span>'),
        d++
    }),
    a = setInterval(b, 9e3),
...
$(function() {
    ...
    $(".level-sub").on("click", function() {
        $(this).next("ul").toggleClass("show")
    }),
    $(".socials").before().on("click", function() {
        $(".socials").toggleClass("scl")
    }),
    ...
    $(".slik-slide6").slick({
        ...
        slidesToShow: 6,
        slidesToScroll: 1,
        prevArrow: '<button class="slide-arrow prev-arrow" aria-label="move item left"><i class="fas fa-chevron-left"></i></button>',
        nextArrow: '<button class="slide-arrow next-arrow" aria-label="move item right"><i class="fas fa-chevron-right"></i></button>',
        responsive: [{
            breakpoint: 1024,
            settings: {
                slidesToShow: 6,
    ...
    $(window).scroll(function() {
        var s = $("#fixtop");
        $(window).scrollTop() >= 200 ? s.addClass("fixedtop") : s.removeClass("fixedtop")
    }),
    $(".ltbox").simpleLightbox(),
    $(".ltboxgal a").simpleLightbox()
});
window.addEventListener('load', (event) => {
    $(".loader").fadeOut()
}
);
//document.addEventListener('touchstart', function(e){console.log('touch passive event');},{passive: true});

var isCaptchaCheked = false;
function onReCaptchaCheck() {
    isCaptchaCheked = true;
    document.getElementById("captchaErr").innerHTML = "";
    document.getElementById("captcha-check").name = "botman";
    ...
    document.getElementById("captcha-check").disabled = "disabled";
}
function sendContactData(evt) {
    if (!isCaptchaCheked) {
        document.getElementById("captchaErr").innerHTML = "Check the captcha";
        return false;
    }
}
